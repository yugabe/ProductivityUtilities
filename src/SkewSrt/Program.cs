...
_ = lines.First(l => TryParseStamps(l, out startStamp, out _));
_ = lines.Last(l => TryParseStamps(l, out endStamp, out _));
...
static bool TryParseStamps(string text, out TimeSpan start, out TimeSpan end)
{
...
static bool TryParseStamp(string text, out TimeSpan value) => TimeSpan.TryParseExact(text, TimeSpanFormat, null, out value);
